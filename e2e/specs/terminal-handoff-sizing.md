...
3. **action:** Open a mobile browser session for the same account and wait for the same terminal to appear in watch mode
   **eval:** The same terminal is visible on mobile as a watcher. The mobile session still sees the same authoritative `cols` and `rows`.
...
5. **action:** On mobile, tap `Fit Here`
   **eval:** The shared terminal is explicitly resized to match the mobile viewport, and the authoritative `cols` and `rows` on the server change.
