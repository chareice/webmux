...
4. **action:** Create a second terminal in a different cwd (e.g. via the API or by launching from another workpath).
   **eval:** A second tab appears in the TabBar. Hovering a tab switches the workspace to that group; the active tab merges visually into the terminal workspace.

5. **action:** Press `⌃B %` (Ctrl+B, then Shift+5).
   **eval:** The active pane splits to the right — the group now shows two panes side by side, and the tab annotation lists the pane terminals. `⌃B "` splits down.
