...
1. **action:** In browser session A, open http://localhost:4317
   **eval:** The desktop workbench loads with the TabBar on top (host switcher, online status, cpu/mem/disk micro-meters, and a "viewing" pill while the session is not the controller). The main area shows the empty state "No terminals yet" (no "Start terminal" button is shown because session A is not yet the controller).
...
6. **action:** In browser session B, right-click the terminal pane and choose "Close pane" from the context menu
   **eval:** The terminal disappears from session B's workspace. The empty state "No terminals yet" is visible again. Session B remains the controller (its TabBar shows the view-only lock, not the pill).
