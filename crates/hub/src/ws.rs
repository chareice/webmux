...
async fn handle_terminal_ws(
    ...
    // them again is idempotent — a no-op if already active. These bytes are
    // generated by the hub (not the machine) and deliberately not counted
    // toward the terminal's output_seq — they are per-connection capability
    // announcements, not part of the PTY history.
    {
