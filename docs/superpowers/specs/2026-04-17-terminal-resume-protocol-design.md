...
`ws.rs:154-161` currently sends `\x1b[?1003h\x1b[?1006h` as a second binary frame after the buffer. Keep this behavior; send on every attach regardless of resume mode. It does not contribute to `next_seq` because it's generated by the hub, not the machine. (Rationale: these escape sequences target the client's xterm, not the server-side PTY.)
...
- **Seq accuracy on output from the hub itself** — if the hub ever emits synthetic bytes into the output stream (currently it doesn't except `MOUSE_ENABLE`), they must either be counted in `next_seq` or explicitly excluded. Current plan: `MOUSE_ENABLE` is not counted (it's a per-connection capability announcement, not part of the PTY history). Enforce this with a comment + test.
