...
internal data class Release(val version: String, val url: String, val size: Long, val digest: String)
...
internal object ReleasePolicy {
    ...
    fun select(releases: JSONArray, current: String, abis: List<String>): Release? {
        ...
        for (i in 0 until releases.length()) {
            ...
            val names = abis.filter { it == "arm64-v8a" || it == "x86_64" }.map { "offdesk-$version-$it.apk" } + "offdesk-$version-universal.apk"
            val asset = names.firstNotNullOfOrNull { name ->
                (0 until assets.length()).map { assets.getJSONObject(it) }.firstOrNull { it.optString("name") == name }
            } ?: continue
            val url = asset.optString("browser_download_url")
            val expected = "https://github.com/zalify/offdesk/releases/download/$tag/${asset.getString("name")}"
            val size = asset.optLong("size")
            val digest = asset.optString("digest")
            if (url != expected || size !in 1..268435456 || !Regex("sha256:[0-9a-f]{64}").matches(digest)) continue
            if (best == null || compare(version, best.version) > 0) best = Release(version, url, size, digest.removePrefix("sha256:"))
        }
        return best
    }
}
