 ...
 */
function redirectTokenToDesktop(jwt) {
    if (Platform.OS !== "web" || typeof window === "undefined")
        return false;
...
// ── Tauri desktop login ──
async function tauriDesktopLogin(onToken) {
    ...
    const unlisten = await listen("oauth-token", (event) => {
        unlisten();
...
export function AuthProvider({ children }) {
    ...
    const value = useMemo(() => ({ user, token, isLoading, isAuthenticated, login, logout }), [user, token, isLoading, isAuthenticated, login, logout]);
