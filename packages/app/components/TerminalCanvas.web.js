...
import { createBookmark, createTerminal, deleteBookmark, destroyTerminal, checkForegroundProcess, eventsWsUrl, getBootstrap, listBookmarks, requestControl, releaseControl, } from "@/lib/api";
...
import { createInitialMainLayout, mainLayoutReducer, } from "@/lib/mainLayoutReducer";
import { storePendingControlRelease, takePendingControlRelease, } from "@/lib/unloadControlRelease";
const OnboardingView = lazy(() => import("./OnboardingView.web").then((module) => ({
    default: module.OnboardingView,
})));
const StatusBar = lazy(() => import("./StatusBar").then((module) => ({ default: module.StatusBar })));
...
export function TerminalCanvas() {
    ...
    const [layout, dispatchLayout] = useReducer(mainLayoutReducer, undefined, createInitialMainLayout);
    const isMobile = useIsMobile();
    const viewportHeight = useVisualViewportHeight();
    const viewportWidth = useViewportWidth();
    const rootHeight = viewportHeight !== null ? `${viewportHeight}px` : "100dvh";
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        void Promise.all(machines.map((m) => {
            const fallback = [
                {
                    id: "local-home",
                    machine_id: m.id,
    ...
    useEffect(() => {
        ...
        if (pending.length === 0)
            return;
        let cancelled = false;
        void Promise.allSettled(pending.map((machineId) => requestControl(machineId, deviceId))).finally(() => {
            if (!cancelled)
                setReconnectGeneration((value) => value + 1);
        });
        return () => {
    ...
    const zoomedTerminalIdRef = useRef(layout.zoomedTerminalId);
    useEffect(() => {
    ...
    const scopeBookmark = layout.selectedWorkpathId === "all"
        ? null
        : bookmarks.find((b) => b.id === layout.selectedWorkpathId) ?? null;
    const scopedTerminals = useMemo(() => {
        if (!activeMachine)
            return [];
        const hostTerminals = terminals.filter((t) => t.machine_id === activeMachine.id);
        if (layout.selectedWorkpathId === "all")
            return hostTerminals;
        if (!scopeBookmark)
            return [];
    ...
    const workpathLabelByMachineAndCwd = useMemo(() => {
        ...
        for (const bm of bookmarks)
            m.set(`${bm.machine_id}::${bm.path}`, bm.label);
        return m;
    }, [bookmarks]);
    const handleCreateTerminal = useCallback(async (machineId, cwd, startupCommand) => {
        if (!deviceId)
            return;
        if (!isMachineController(machineId))
            return;
        const newTerminal = await createTerminal(machineId, cwd, deviceId, startupCommand);
        const match = bookmarks.find((b) => b.machine_id === machineId && b.path === cwd);
        dispatchLayout({
    ...
    const handleRequestControl = useCallback(async (machineId) => {
        if (!deviceId)
            return;
    ...
    const handleReleaseControl = useCallback(async (machineId) => {
        if (!deviceId)
            return;
        ...
        setBrowserState((prev) => ({
            ...
            controlLeases: next.controller_device_id
                ...
                : Object.fromEntries(Object.entries(prev.controlLeases).filter(([key]) => key !== machineId)),
        }));
    }, [deviceId]);
    const handleDestroyTerminal = useCallback(async (terminal) => {
        if (!deviceId)
            return;
        if (!isMachineController(terminal.machine_id))
            return;
        try {
            const result = await checkForegroundProcess(terminal.machine_id, terminal.id);
    ...
    const confirmClosePending = useCallback(async () => {
        if (!closeConfirmation || !deviceId)
            return;
        const { terminal } = closeConfirmation;
        setCloseConfirmation(null);
        await destroyTerminal(terminal.machine_id, terminal.id, deviceId);
    }, [closeConfirmation, deviceId]);
    const handleZoomTerminal = useCallback((id) => {
        dispatchLayout({ type: "ZOOM_TERMINAL", terminalId: id });
    ...
    const handleSelectWorkpath = useCallback((id) => {
    ...
    const handleConfirmAddDirectory = useCallback(async (machineId, path) => {
    ...
    const handleSelectWorkpathByIndex = useCallback((index) => {
        if (index === 0) {
    ...
    const currentWorkpathTerminals = useCallback(() => scopedTerminals, [scopedTerminals]);
    useShortcuts({
    ...
    // ---- render ----
    const scopeLabel = layout.selectedWorkpathId === "all"
        ? "All"
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        {showSettings ? (<Suspense fallback={null}>
          ...
          </Suspense>) : machines.length === 0 ? (<Suspense fallback={null}>
            <OnboardingView />
          </Suspense>) : isMobile ? (<MobileWorkbench machines={machines} activeMachineId={activeMachineId} controlLeases={controlLeases} deviceId={deviceId} machineStats={machineStats} bookmarks={bookmarks} terminals={terminals} selectedWorkpathId={layout.selectedWorkpathId} canCreateTerminal={isActiveController} onSelectMachine={setActiveMachineId} onSelectWorkpath={handleSelectWorkpath} onOpenTerminal={handleZoomTerminal} onNewTerminal={handleNewTerminalFromHeader} onRequestControl={handleRequestControl} onReleaseControl={handleReleaseControl} onOpenSettings={() => setShowSettings(true)}/>) : (<>
            {/* Rail (drawer on tight screens) */}
            {railOpen && (<Rail width={tight ? Math.min(viewportWidth - 40, 260) : 248} machines={machines} activeMachineId={activeMachineId} controlLeases={controlLeases} deviceId={deviceId} machineStats={machineStats} bookmarks={bookmarks} terminals={terminals} selectedWorkpathId={layout.selectedWorkpathId} canCreateTerminal={isActiveController} addDirectoryOpen={addDirectoryOpen} onSelectMachine={(id) => {
                    ...
                    if (tight)
                        setRailOpen(false);
                }} onOpenAddDirectory={() => setAddDirectoryOpen(true)} onCloseAddDirectory={() => setAddDirectoryOpen(false)} onConfirmAddDirectory={handleConfirmAddDirectory} onRemoveBookmark={handleRemoveBookmark} onOpenSettings={() => setShowSettings(true)} onCollapse={() => setRailOpen(false)}/>)}
            {tight && railOpen && (<div onClick={() => setRailOpen(false)} style={{
                    position: "absolute",
                    inset: 0,
            ...
            <main style={{
              ...
              <WorkbenchHeader scopeLabel={scopeLabel} hostName={activeMachine?.name ?? "webmux"} isController={isActiveController} terminalCount={scopedTerminals.length} stats={activeStats} viewportWidth={viewportWidth} canCreateTerminal={isActiveController} railOpen={railOpen} onOpenRail={() => setRailOpen(true)} onNewTerminal={isActiveController ? handleNewTerminalFromHeader : undefined} onReleaseControl={isActiveController && activeMachine
                ? () => handleReleaseControl(activeMachine.id)
                : undefined} onRequestControl={!isActiveController && activeMachine
                ? () => handleRequestControl(activeMachine.id)
                : undefined}/>

              {scopedTerminals.length === 0 ? (<EmptyState scopeLabel={scopeLabel} canCreate={isActiveController} onNewTerminal={handleNewTerminalFromHeader}/>) : (<div data-testid="workbench-grid" style={{
                    flex: 1,
                    overflow: "auto",
                    padding: 14,
                    display: "grid",
                  ...
                  {scopedTerminals.map((t) => (<TerminalGridCard key={t.id} terminal={t} isController={isMachineController(t.machine_id)} deviceId={deviceId ?? ""} workpathLabel={layout.selectedWorkpathId === "all"
                        ? workpathLabelByMachineAndCwd.get(`${t.machine_id}::${t.cwd}`)
                        : undefined} onExpand={handleZoomTerminal} onDestroy={handleDestroyTerminal}/>))}
                </div>)}
            </main>
          </>)}
      </div>

      {statusBarVisible && machines.length > 0 && (<Suspense fallback={null}>
          <StatusBar machines={machines} activeMachineId={activeMachineId} onSelectMachine={setActiveMachineId} machineStats={machineStats} isMobile={isMobile} isController={isActiveController} onRequestControl={handleRequestControl} onReleaseControl={handleReleaseControl}/>
        </Suspense>)}

      {expandedTerminal && (<ExpandedTerminal terminal={expandedTerminal} siblings={scopedTerminals.length > 0 ? scopedTerminals : [expandedTerminal]} isController={isMachineController(expandedTerminal.machine_id)} deviceId={deviceId ?? ""} isMobile={isMobile} onClose={handleUnzoom} onPick={handleZoomTerminal} onDestroy={handleDestroyTerminal} onRequestControl={handleRequestControl} onReleaseControl={handleReleaseControl}/>)}

      {closeConfirmation && (<Suspense fallback={null}>
          <ConfirmDialog open title="Close terminal?" message={`"${closeConfirmation.processName}" is still running in this terminal. Closing the terminal will terminate it.`} confirmLabel="Close terminal" cancelLabel="Cancel" variant="danger" onConfirm={confirmClosePending} onCancel={() => setCloseConfirmation(null)}/>
        </Suspense>)}
    </div>);
}
function EmptyState({ scopeLabel, canCreate, onNewTerminal, }) {
    return (<div style={{
            flex: 1,
            display: "flex",
            alignItems: "center",
      ...
      <div style={{ textAlign: "center" }}>
        ...
        {canCreate && (<button data-testid="empty-new-terminal" onClick={onNewTerminal} style={{
                marginTop: 14,
