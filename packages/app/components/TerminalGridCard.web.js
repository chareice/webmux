...
function TerminalGridCardComponent(props) {
    const { terminal, isController, deviceId, workpathLabel, onExpand, onDestroy, } = props;
    ...
    const wsUrl = terminal.reachable
        ...
        : null;
    return (<div data-testid={`grid-card-${terminal.id}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onClick={() => onExpand(terminal.id)} style={{
      ...
      <div style={{
        ...
        <div style={{
        ...
        }} title="Expand" aria-label="Expand terminal" style={iconBtn}>
            <Expand size={12}/>
          </button>
          <button onClick={(e) => e.stopPropagation()} title="More" aria-label="More actions" style={iconBtn}>
          ...
          <button onClick={(e) => {
            ...
            if (!isController)
                return;
            onDestroy(terminal);
        }} title={isController ? "Close" : "View only — cannot close"} aria-label="Close terminal" disabled={!isController} style={{
            ...iconBtn,
      ...
      <div style={{
        ...
        {terminal.reachable && wsUrl && (<Suspense fallback={null}>
            <div style={{
                width: "100%",
                height: "100%",
                pointerEvents: "none",
              ...
              <LiveTerminalView machineId={terminal.machine_id} terminalId={terminal.id} wsUrl={wsUrl} cols={terminal.cols} rows={terminal.rows} displayMode="card" isController={isController} canResizeTerminal={false} style={{
        ...
        <div style={{
            ...
            top: 0,
            height: 14,
            background: "linear-gradient(rgba(5,6,10,1), rgba(5,6,10,0))",
            pointerEvents: "none",
        }}/>
      </div>
      ...
      <div style={{
        ...
        {workpathLabel && (<span style={{ color: colors.fg2, flexShrink: 0 }}>
            {workpathLabel}
          </span>)}
      </div>
    </div>);
