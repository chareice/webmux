...
function measureTerminalSurface(container) {
    ...
    const width = Math.max(grid?.scrollWidth ?? 0, grid?.clientWidth ?? 0, container.scrollWidth, container.clientWidth);
    const height = Math.max(grid?.scrollHeight ?? 0, grid?.clientHeight ?? 0, container.scrollHeight, container.clientHeight);
    return { width, height };
}
const WTERM_THEME_STYLE = {
