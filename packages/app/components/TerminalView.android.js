 ...
 */
export const TerminalView = forwardRef(function TerminalView({ machineId, terminalId, wsUrl, cols, rows, isController, canResizeTerminal, onTitleChange, style, }, ref) {
    ...
    const postToWebView = useCallback((msg) => {
        webViewRef.current?.postMessage(JSON.stringify(msg));
    }, []);
    const sendResizeToHub = useCallback((dims) => {
        ...
        ws.onmessage = (event) => {
            void Promise.resolve(event.data)
                ...
                if (payload instanceof ArrayBuffer) {
                    writeToTerminal(decoderRef.current.decode(new Uint8Array(payload), {
                        stream: true,
                    }));
                    return;
                }
                if (payload && typeof payload.arrayBuffer === "function") {
                    ...
                    writeToTerminal(decoderRef.current.decode(new Uint8Array(buffer), {
                        stream: true,
                    }));
                }
            })
    ...
    return (<View style={[styles.container, style]}>
        <WebView ref={webViewRef} source={{ html: TERMINAL_HTML }} style={styles.webview} onMessage={handleMessage} javaScriptEnabled domStorageEnabled 
