...
export function ExtendedKeyBar({ onKey, onToggleKeyboard, keyboardVisible, isController, }) {
    return (<div style={{
            display: 'flex',
            alignItems: 'center',
      ...
      {isController && (<button onClick={onToggleKeyboard} style={{
                width: 44,
                height: 44,
                display: 'flex',
                alignItems: 'center',
      ...
      <div style={{
        ...
        {KEY_GROUPS.map((group, gi) => (<div key={gi} style={{
                display: 'flex',
                gap: 2,
                padding: '0 2px',
            ...
            {group.map(key => (<button key={key.label} onClick={() => {
                    if (!isController)
                        return;
                    onKey(key.data);
                }} disabled={!isController} style={{
