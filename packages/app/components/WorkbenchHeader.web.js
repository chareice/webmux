...
function WorkbenchHeaderComponent(props) {
    const { scopeLabel, hostName, isController, terminalCount, stats, viewportWidth, canCreateTerminal, railOpen, onOpenRail, onNewTerminal, onReleaseControl, onRequestControl, } = props;
    const compact = viewportWidth < 1180;
    const tight = viewportWidth < 820;
    const controlCopy = getTerminalControlCopy(isController);
    return (<header data-testid="workbench-header" style={{
            display: "flex",
      ...
      <div style={{
        ...
        {!railOpen && (<button onClick={onOpenRail} title="Open sidebar" aria-label="Open sidebar" style={{
                width: 24,
                height: 24,
                borderRadius: 6,
                display: "inline-flex",
        ...
        {!tight && (<span style={{ height: 18, width: 1, background: colors.line, flexShrink: 0 }}/>)}
        {!tight && <ControllingPill isController={isController}/>}

        {compact && <div style={{ flex: 1 }}/>}
        {compact && (<StatChips stats={stats} terminals={terminalCount} compact/>)}
      </div>

      {!compact && <div style={{ flex: 1 }}/>}
      {!compact && <StatChips stats={stats} terminals={terminalCount}/>}
      {!compact && (<span style={{ height: 22, width: 1, background: colors.line, flexShrink: 0 }}/>)}
      ...
      <div style={{
        ...
        {canCreateTerminal && onNewTerminal && (<button data-testid="workbench-new-terminal" onClick={onNewTerminal} title="New terminal" style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
        ...
        {isController && onReleaseControl ? (<button data-testid="workbench-stop-control" onClick={onReleaseControl} title={controlCopy.toggleLabel} style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
          ...
          </button>) : onRequestControl ? (<button data-testid="workbench-request-control" onClick={onRequestControl} title={controlCopy.toggleLabel} style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
                padding: tight ? "6px 9px" : "6px 10px",
...
/* ---------- Subcomponents ---------- */
function Breadcrumb({ host, scope }) {
    return (<div style={{
            display: "flex",
            alignItems: "center",
            gap: 8,
      ...
      <span style={{ color: colors.fg3, fontSize: 12, whiteSpace: "nowrap" }}>
        {host}
      </span>
...
function ControllingPill({ isController }) {
    if (isController) {
        return (<span style={{
                display: "inline-flex",
                alignItems: "center",
...
function StatChips({ stats, terminals, compact, }) {
    ...
    return (<div style={{
      ...
      <StatChip label="CPU" value={`${cpu}%`} series={compact ? null : cpuSeries}/>
      <StatChip label="MEM" value={`${mem}%`} series={compact ? null : memSeries}/>
      {!compact && (<StatChip label="TERM" value={String(terminals)} series={null}/>)}
    </div>);
}
function StatChip({ label, value, series, }) {
    return (<span style={{
            display: "inline-flex",
            alignItems: "center",
            gap: 6,
      ...
      {series && (<Sparkline data={series} width={44} height={14} color={colors.fg2} fill="rgba(144, 146, 151, 0.12)"/>)}
    </span>);
}
...
function Chevron({ right, size = 14 }) {
    return (<svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" style={{ display: "block" }}>
      <polyline points={right ? "9 6 15 12 9 18" : "15 6 9 12 15 18"}/>
    </svg>);
}
/* ---------- Small sparkline + deterministic mock series ---------- */
export function Sparkline({ data, width = 64, height = 18, color = "currentColor", fill, }) {
...
export function mockSeries(seed, n = 24, floor = 0.1, ceil = 0.9) {
    const out = [];
    let x = seed;
    for (let i = 0; i < n; i++) {
