...
function MobileWorkbenchComponent(props) {
    ...
    return (<div data-testid="mobile-workbench" style={{
      ...
      <div style={{
        ...
        {tab === "hosts" && (<HostsPage machines={machines} activeMachineId={activeMachineId} controlLeases={controlLeases} deviceId={deviceId} bookmarks={bookmarks} terminals={terminals} selectedWorkpathId={selectedWorkpathId} onAddMachine={() => setAddMachineOpen(true)} onSelectMachine={onSelectMachine} onSelectWorkpath={(id) => {
                onSelectWorkpath(id);
                setTab("terminals");
            }}/>)}
      ...
      {menuSheet && (<Sheet onClose={() => setMenuSheet(false)}>
          <MenuRow icon={<Plus size={17}/>} label="Add host" onClick={() => {
                setMenuSheet(false);
...
/* ---------- Pages ---------- */
function HostsPage({ machines, activeMachineId, controlLeases, deviceId, bookmarks, terminals, selectedWorkpathId, onAddMachine, onSelectMachine, onSelectWorkpath, }) {
    ...
    return (<div style={{ height: "100%", overflow: "auto", padding: "8px 0 16px" }}>
      ...
      <div style={{ padding: "0 16px 10px" }}>
        <button data-testid="mobile-add-machine" onClick={onAddMachine} style={{
            width: "100%",
            display: "inline-flex",
            alignItems: "center",
            justifyContent: "center",
